...
def train_epoch_rnn(dataset, encoder, decoder, encoder_optimizer,
                    decoder_optimizer, criterion, device, batch_size):
    encoder.train()
    decoder.train()
    running_loss = 0
...
def main() -> int:
    ...
    if configs['model_type'] == 'transformer':
        transformer = Seq2SeqTransformer(NUM_ENCODER_LAYERS,
                                         NUM_DECODER_LAYERS, EMB_SIZE, NHEAD,
        ...
        for epoch in tqdm(range(1, NUM_EPOCHS + 1)):
            start_time = timer()
            train_loss = train_epoch(transformer, optimizer, train_data,
                                     criterion, BATCH_SIZE, device)
            end_time = timer()
            val_loss = evaluate(transformer, val_data, criterion, BATCH_SIZE,
                                device)
            print((
        ...
        encoder = EncoderRNN(SRC_VOCAB_SIZE, EMB_SIZE, EMB_SIZE,
                             device).to(device)
        decoder = DecoderRNN(TGT_VOCAB_SIZE, EMB_SIZE, EMB_SIZE,
                             TGT_VOCAB_SIZE, device).to(device)
        ...
        for epoch in tqdm(range(1, NUM_EPOCHS + 1)):
            start_time = timer()
            train_loss = train_epoch_rnn(train_data, encoder, decoder,
